...
CEPH_RADOS_API std::ostream &operator<<(std::ostream &out,
                                        const librados::ListObject &lop);
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const librados::ObjectCursor &oc);
...
CEPH_RADOS_API std::ostream &operator<<(std::ostream &os,
                                        const librados::ObjectCursor &oc);

class CEPH_RADOS_API NObjectIterator
    : public std::iterator<std::forward_iterator_tag, ListObject> {
public:
  ...
  const ListObject *operator->() const;
  NObjectIterator &
  operator++(); //< Preincrement; errors are thrown as exceptions
  NObjectIterator
  operator++(int); //< Postincrement; errors are thrown as exceptions
  friend class IoCtx;
  friend class librados::NObjectIteratorImpl;
  ...
  /// move the iterator to a given cursor position. errors are thrown as
  /// exceptions
...
public:
   ...
   *
   * assertions has the form of mappings from keys to (comparison rval,
   * assertion) The assertion field may be CEPH_OSD_CMPXATTR_OP_[GT|LT|EQ].
   *
   ...
   */
  void
  omap_cmp(const std::map<std::string, std::pair<bufferlist, int>> &assertions,
           int *prval);
...
public:
  ...
  void set_chunk(uint64_t src_offset, uint64_t src_length,
                 const IoCtx &tgt_ioctx, std::string tgt_oid,
...
public:
   ...
   * @param prval [out] place error code in prval upon completion
   */
  void omap_get_vals(const std::string &start_after, uint64_t max_return,
   ...
   * @param prval [out] place error code in prval upon completion
   */
  void omap_get_vals2(const std::string &start_after,
                      const std::string &filter_prefix, uint64_t max_return,
                      std::map<std::string, bufferlist> *out_vals, bool *pmore,
                      int *prval);
   ...
   * @param prval [out] place error code in prval upon completion
   */
  void omap_get_keys(const std::string &start_after, uint64_t max_return,
   ...
   * @param prval [out] place error code in prval upon completion
   */
  void omap_get_keys2(const std::string &start_after, uint64_t max_return,
   ...
   *
   * @param out_watchers [out] place returned values in out_watchers on
   * completion
   * @param prval [out] place error code in prval upon completion
   */
...
public:
  ...
  int writesame(const std::string &oid, bufferlist &bl, size_t write_len,
                uint64_t off);
  ...
  int mapext(const std::string &o, uint64_t off, size_t len,
  ...
  int omap_get_vals(const std::string &oid, const std::string &start_after,
                    uint64_t max_return,
                    std::map<std::string, bufferlist> *out_vals);
  int omap_get_vals2(const std::string &oid, const std::string &start_after,
                     uint64_t max_return,
                     std::map<std::string, bufferlist> *out_vals, bool *pmore);
  int omap_get_vals(const std::string &oid, const std::string &start_after,
                    const std::string &filter_prefix, uint64_t max_return,
  ...
  int omap_get_vals2(const std::string &oid, const std::string &start_after,
                     const std::string &filter_prefix, uint64_t max_return,
                     std::map<std::string, bufferlist> *out_vals, bool *pmore);
  int omap_get_keys(const std::string &oid, const std::string &start_after,
                    uint64_t max_return, std::set<std::string> *out_keys);
  ...
  int lock_exclusive(const std::string &oid, const std::string &name,
                     const std::string &cookie, const std::string &description,
                     struct timeval *duration, uint8_t flags);

  int lock_shared(const std::string &oid, const std::string &name,
                  const std::string &cookie, const std::string &tag,
                  const std::string &description, struct timeval *duration,
                  uint8_t flags);
  ...
  int object_list(const ObjectCursor &start, const ObjectCursor &finish,
                  const size_t result_count, const bufferlist &filter,
                  std::vector<ObjectItem> *result, ObjectCursor *next);

  /// Generate cursors that include the N cpp of Mth slice of the pool
  void object_list_slice(const ObjectCursor start, const ObjectCursor finish,
  ...
  int aio_read(const std::string &oid, AioCompletion *c, bufferlist *pbl,
   ...
   */
  int aio_cmpext(const std::string &oid, librados::AioCompletion *c,
  ...
  int aio_write_full(const std::string &oid, AioCompletion *c,
                     const bufferlist &bl);
  int aio_writesame(const std::string &oid, AioCompletion *c,
  ...
  int aio_getxattr(const std::string &oid, AioCompletion *c, const char *name,
                   bufferlist &bl);
  int aio_getxattrs(const std::string &oid, AioCompletion *c,
                    std::map<std::string, bufferlist> &attrset);
  int aio_setxattr(const std::string &oid, AioCompletion *c, const char *name,
                   bufferlist &bl);
  ...
  int aio_stat(const std::string &oid, AioCompletion *c, uint64_t *psize,
               time_t *pmtime);
  int aio_stat2(const std::string &oid, AioCompletion *c, uint64_t *psize,
                struct timespec *pts);
  ...
  int aio_operate(const std::string &oid, AioCompletion *c,
                  ObjectReadOperation *op, int flags, bufferlist *pbl);
  int aio_operate(const std::string &oid, AioCompletion *c,
                  ObjectReadOperation *op, int flags, bufferlist *pbl,
                  const blkin_trace_info *trace_info);
  ...
  int watch3(const std::string &o, uint64_t *handle, librados::WatchCtx2 *ctx,
             uint32_t timeout);
   ...
   * @returns 0 on success, negative error code on failure
   */
  int set_alloc_hint(const std::string &o, uint64_t expected_object_size,
                     uint64_t expected_write_size);
  int set_alloc_hint2(const std::string &o, uint64_t expected_object_size,
                      uint64_t expected_write_size, uint32_t flags);
  ...
  int get_object_hash_position2(const std::string &oid,
                                uint32_t *hash_position);
  int get_object_pg_hash_position2(const std::string &oid,
  ...
  int application_enable_async(const std::string &app_name, bool force,
...
private:
  ...
  friend class Rados; // Only Rados can use our private constructor to create
                      // IoCtxes.
  friend class libradosstriper::RadosStriper; // Striper needs to see our
                                              // IoCtxImpl
...
public:
  ...
  int service_daemon_register(
      ...
      const std::map<std::string, std::string>
          &metadata); ///< static metadata about daemon
  ...
  int pool_create_async(const char *name, uint64_t auid, PoolAsyncCompletion *c)
      __attribute__((deprecated));
  int pool_create_async(const char *name, uint64_t auid, uint8_t crush_rule,
                        PoolAsyncCompletion *c) __attribute__((deprecated));
  int pool_create_with_rule_async(const char *name, uint8_t crush_rule,
                                  PoolAsyncCompletion *c);
  ...
  int mon_command(std::string cmd, const bufferlist &inbl, bufferlist *outbl,
                  std::string *outs);
  int mgr_command(std::string cmd, const bufferlist &inbl, bufferlist *outbl,
                  std::string *outs);
  ...
  int get_pool_stats(std::list<std::string> &v, std::string &category,
                     std::map<std::string, stats_map> &stats);
   ...
   * pool aio
   *
   * It is up to the caller to release the completion handler, even if the
   * pool_create_async() and/or pool_delete_async() fails and does not send the
   * async request
   */
  ...
  static AioCompletion *aio_create_completion(void *cb_arg,
