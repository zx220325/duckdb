...
static const bool is_nexus = getenv("NEXUS_ENVIRONMENT") != nullptr;

idx_t DBConfig::GetSystemMaxThreads(FileSystem &fs) {
	auto GetSystemMaxThreadsImpl = [](FileSystem &fs) -> idx_t {
#ifndef DUCKDB_NO_THREADS
		idx_t physical_cores = is_nexus ? 1 : std::thread::hardware_concurrency();
#ifdef __linux__
	...
	return MinValue<idx_t>(GetSystemMaxThreadsImpl(fs), 1);
}
